...
public record struct ClientOptions()
{
    ...
    Lazy<string> _baseUrl = new(() =>
        Environment.GetEnvironmentVariable("PHOEBE_BASE_URL") ?? EnvironmentUrl.Production
    );
    ...
    /// <see cref="IPhoebeClient.WithRawResponse"/>, don't perform validation until deserialization
    /// is triggered.</para>
    /// </summary>
